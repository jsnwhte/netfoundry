	...
	process: function(node, msg) {
		switch(msg.topic) {
			...
			case constants.URI_NOTIF_LINK_REQUEST:
				node.receiveLinkAddRequest(msg.fromUri, msg.linkDirection);
				break;
			
			case constants.URI_NOTIF_LINK_REQUEST_ACK:
				node.receiveLinkAddAck(msg.fromUri, msg.linkDirection, msg.ack);
				break;
			
			case constants.URI_NOTIF_LINK_REMOVE:
				node.receiveLinkRemoveNotification(msg.fromUri, msg.linkDirection);
				break;
