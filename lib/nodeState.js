...
function NodeState(arg)
{
	...
	this.receiveLinkAddRequest = function(linkUri, linkType) {
		...
		switch (linkType) {
			...
			case constants.OUTGOING:
				...
				var ack = processor.call(_self, linkUri, myLinkType);
	...
	*/
	...
	function _addLinkRequest(linkUri, linkType) {
		...
		if (_getLinkArray(linkType).indexOf(linkUri) == -1 && _self.pendingLinks.indexOf(pendUri) == -1) {
			...
			_self.emit('linkRequest', _self.uri, linkUri, linkType);
		}
	}

	function _removeLinkRequest(linkUri, linkType) {
		if (_removeLink(linkUri, linkType)) {
			_self.emit('linkRemove', _self.uri, linkUri, linkType);
		}
	}
